...
wrapper.authenticate = function(options) {
  ...
  return function(request, reply) {
    function validateCallback(err, profile, loggedOut) {
      ...
      // Passport specific verify method:
      // self._verify(profile, verified);
      //
      // Passes profile back to next middleware
      // function, to check credentials, then
      // expect it to call `verified()`
      //
      // Should instead set `request.auth`?
