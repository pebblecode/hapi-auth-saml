...
wrapper.authenticate = function(options) {
  ...
  return function(request, reply) {
    function validateCallback(err, profile, loggedOut) {
      request.log(['debug', 'validateCallback'], profile);
      ...
      var verified = function (err, user, info) {
        request.log(['debug', 'verified', 'user'], user);
        request.log(['debug', 'verified', 'info'], info);
      ...
      // valid or not.
      request.log(['debug', 'pre-verifyFunc'], profile);
      options.verifyFunc(profile, verified);
    }

    function redirectIfSuccess(err, url) {
      request.log(['debug', 'redirectIfSuccess'], url);
